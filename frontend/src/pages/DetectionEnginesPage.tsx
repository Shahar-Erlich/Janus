    ...
    <div className="page-stack">
      <section className="page-hero-row events-hero-row">
        <div>
          ...
          <p className="page-section-subtitle">Real-time inspection layer status and performance metrics across the security stack.</p>
        </div>
        {/* <button className="primary-button"><RefreshCw size={16} /> Update All</button> */}
      </section>

      {error ? <div className="placeholder-box">{error}</div> : null}
      {loading && engines.length === 0 ? <div className="placeholder-box">Loading engine telemetry…</div> : null}

      <section className="engine-grid">
        {engines.map((engine) => (
          <SectionCard key={engine.name} title={engine.name} className="engine-card" subtitle={engine.statusText}>
            <div className={`engine-status engine-status-${engine.statusTone}`}>{engine.statusText}</div>
            ...
            <div className="engine-latency-row">
              ...
              <div className="engine-mini-meter">
                <div className="engine-mini-meter-bar" style={{ width: `${engine.meterValue}%` }} />
              </div>
            </div>
          </SectionCard>
        ))}
          ...
          {engines.map((engine) => (
            <div key={engine.name} className="throughput-row">
              ...
              <div className="throughput-track">
                <div className="throughput-bar" style={{ width: `${engine.meterValue}%` }} />
              </div>
            </div>
          ))}
        </div>
